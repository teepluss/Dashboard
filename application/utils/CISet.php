 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::merge
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::filter
 */
	public static function filter(array $var) {
		foreach ($var as $k => $v) {
			if (is_array($v)) {
				$var[$k] = CISet::filter($v);
			}
		}
...
/**
 * CISet::filter callback function
 *
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::pushDiff
 */
		...
		if (!empty($array) && !empty($array2)) {
			foreach ($array2 as $key => $value) {
				if (!array_key_exists($key, $array)) {
					...
					if (is_array($value)) {
						$array[$key] = CISet::pushDiff($array[$key], $array2[$key]);
					}
				}
			}
		}
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::map
 */
		...
		if (is_array($array)) {
			...
			foreach ($array as $key => $value) {
				...
				if (is_numeric($key)) {
					...
					$out[$key] = CISet::_map($value, $class);
					if (is_object($out[$key])) {
						if ($primary !== true && is_array($value) && CISet::countDim($value, true) === 2) {
					...
					if ($primary === true) {
						...
						foreach ($value as $key2 => $value2) {
							$out->{$key2} = CISet::_map($value2, true);
						}
					} else {
						if (!is_numeric($key)) {
							$out->{$key} = CISet::_map($value, true, $key);
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::numeric
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::enum
 */
	public static function enum($select, $list = null) {
		...
		$return = null;
		$list = CISet::normalize($list, false);
 ...
 * @param array $keys An array containing one or more CISet::extract()-style key paths
 * @return array An array of strings extracted from $keys and formatted with $format
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::format
 */
	public static function format($data, $format, $keys) {
		...
		for ($i = 0; $i < $count; $i++) {
			$extracted[] = CISet::extract($data, $keys[$i]);
		}
		$out = array();
		$data = $extracted;
		$count = count($data[0]);
 ...
 * is delegated to CISet::classicExtract(). Also the $path and $data arguments are
 * reversible.
 *
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::extract
 */
	public static function extract($path, $data = null, $options = array()) {
		...
		if (strpos($path, '/') === false) {
			return CISet::classicExtract($data, $path);
		}
		if (empty($data)) {
			return array();
		}
		...
		if (!isset($contexts[0])) {
			...
			if ((is_array($current) && count($data) < 1) || !is_array($current) || !CISet::numeric(array_keys($data))) {
				$contexts = array($data);
			}
		}
		...
		do {
			...
			foreach ($contexts as $key => $context) {
				...
				if ($token === '..') {
					...
					$context['item'] = CISet::extract($parent, $data);
			...
			if ($conditions) {
				foreach ($conditions as $condition) {
					...
					foreach ($matches as $i => $match) {
						if (CISet::matches(array($condition), $match['item'], $i + 1, $length)) {
							$filtered[$i] = $match;
						}
					}
					$matches = $filtered;
 ...
 * @return boolean
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::matches
 */
	public static function matches($conditions, $data = array(), $i = null, $length = null) {
		...
		if (is_string($conditions)) {
			return !!CISet::extract($conditions, $data);
		}
		foreach ($conditions as $condition) {
			if ($condition === ':last') {
				if ($i != $length) {
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::classicExtract
 */
				...
				foreach ($data as $j => $val) {
					if (is_int($j)) {
						...
						if (empty($tmpPath)) {
							...
							$tmp[] = CISet::classicExtract($val, $tmpPath);
						}
					}
				}
				return $tmp;
				...
				foreach ($data as $j => $val) {
					if (preg_match('/^' . $pattern . '/s', $j) !== 0) {
						...
						if (empty($tmpPath)) {
							...
							$tmp[$j] = CISet::classicExtract($val, $tmpPath);
						}
					}
				}
				return $tmp;
 ...
 * @return array
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::insert
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::remove
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::check
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::diff
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::contains
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::countDim
 */
		...
		if ($all) {
			...
			if (is_array($array) && reset($array) !== false) {
				foreach ($array as $value) {
					$depth[] = CISet::countDim($value, true, $count + 1);
				}
			}
			$return = max($depth);
		} else {
			if (is_array(reset($array))) {
				$return = CISet::countDim(reset($array)) + 1;
			} else {
				$return = 1;
			}
		}
 ...
 * @return array
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::normalize
 */
 ...
 * to null (useful for CISet::merge). You can optionally group the values by what is obtained when
 ...
 * @return array Combined array
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::combine
 */
	public static function combine($data, $path1 = null, $path2 = null, $groupPath = null) {
		...
		if (is_array($path1)) {
			...
			$keys = CISet::format($data, $format, $path1);
		} else {
			$keys = CISet::extract($data, $path1);
		}
		if (empty($keys)) {
			return array();
		}
		...
		if (!empty($path2) && is_array($path2)) {
			...
			$vals = CISet::format($data, $format, $path2);
		} elseif (!empty($path2)) {
			$vals = CISet::extract($data, $path2);
		} else {
			$count = count($keys);
			for ($i = 0; $i < $count; $i++) {
				$vals[$i] = null;
		...
		if ($groupPath != null) {
			$group = CISet::extract($data, $groupPath);
			if (!empty($group)) {
				$c = count($keys);
				for ($i = 0; $i < $c; $i++) {
					if (!isset($group[$i])) {
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::reverse
 */
	public static function reverse($object) {
		...
		if ($object instanceof SimpleXMLElement) {
			...
			foreach ($keys as $key => $value) {
				if (is_array($value)) {
					$new[$key] = (array)CISet::reverse($value);
				} else {
					// @codingStandardsIgnoreStart Legacy junk
					if (isset($value->_name_)) {
						$new = array_merge($new, CISet::reverse($value));
					} else {
						$new[$key] = CISet::reverse($value);
					}
					// @codingStandardsIgnoreEnd
				}
			}
 ...
 * @return array
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::flatten
 */
	public static function flatten($data, $separator = '.') {
		...
		foreach ($data as $key => $val) {
			if (is_array($val)) {
				$result += (array)CISet::flatten($val, array(
					'separator' => $separator,
					'path' => $path . $key
				));
			} else {
	...
	protected static function _flatten($results, $key = null) {
		...
		foreach ($results as $k => $r) {
			...
			if (is_array($r) && !empty($r)) {
				$stack = array_merge($stack, CISet::_flatten($r, $id));
			} else {
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::sort
 */
	public static function sort($data, $path, $dir) {
		...
		$result = CISet::_flatten(CISet::extract($data, $path));
		list($keys, $values) = array(CISet::extract($result, '{n}.id'), CISet::extract($result, '{n}.value'));
 ...
 * array extracted by a CISet::extract() compatible path.
 *
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CISet::apply
 */
	public static function apply($path, $data, $callback, $options = array()) {
		$defaults = array('type' => 'pass');
		$options = array_merge($defaults, $options);
		$extracted = CISet::extract($path, $data);
 ...
 */
	public static function nest($data, $options = array()) {
		...
		$return = $idMap = array();
		$ids = CISet::extract($data, $options['idPath']);
		...
		foreach ($data as $result) {
			...
			$id = CISet::get($result, $idKeys);
			$parentId = CISet::get($result, $parentKeys);
		...
		if ($options['root']) {
			...
			$root = CISet::get($return[0], $parentKeys);
		}
